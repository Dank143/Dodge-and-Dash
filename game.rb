...
module Path
  ...
  FONTS =  File.join(SOURCE, 'media/fonts/')
  IMAGES = File.join(SOURCE, 'media/images/')
  SOUNDS = File.join(SOURCE, 'media/sounds/')
  DATA = File.join(SOURCE, 'data/')
  LANGS = File.join(SOURCE, 'langs/')
end
...
CARS = [
  [Path::IMAGES + 'car.png'],
  [Path::IMAGES + 'audi.png'],
  [Path::IMAGES + 'black_viper.png'],
]
...
class GameWindow < Gosu::Window
  ...
  def initialize # Set program name, screen dimension and states
    super WIDTH, HEIGHT
    self.caption = 'Street Racer'
    @states = [
  ...
  def load_metadata # Overwrite json data
  ...
  def toggle_music(song = Nil, looping = false) # Create music loop
