# This class contains the speedometer properties.
class Speedometer
  MAX_ANGLE = 75
  MIN_ANGLE = -90
  ...
  def angle(player)
    old_range = player.speed_limit - player.speed_minumum
    new_range = MAX_ANGLE - MIN_ANGLE
    ((player.speed - player.speed_minumum) * new_range) / old_range + MIN_ANGLE
  end
